...
ASIOTime *bufferSwitchTimeInfo(
    ...
    // finally if the driver supports the ASIOOutputReady() optimization, do it
    // here, all data are in place
...
long asioMessages(long selector, long value, void *message, double *opt) {
    ...
    case kAsioResetRequest:
        // defer the task and perform the reset of the driver during the next
        // "safe" situation You cannot reset the driver right now, as this code
        // is called from the driver. Reset the driver is done by completely
        // destruct is. I.e. ASIOStop(), ASIODisposeBuffers(), Destruction
        // Afterwards you initialize the driver again.
        // asioDriverInfo.stopped;  // In this sample the processing will just
        // stop
        ret = 1L;
        break;
    case kAsioResyncRequest:
        // This informs the application, that the driver encountered some non
        // fatal data loss. It is used for synchronization purposes of different
        // media. Added mainly to work around the Win16Mutex problems in Windows
        // 95/98 with the Windows Multimedia system, which could loose data
        // because the Mutex was hold too long by another thread. However a
        // driver can issue it in other situations, too.
        ret = 1L;
        break;
    case kAsioLatenciesChanged:
        // This will inform the host application that the drivers were latencies
        // changed. Beware, it this does not mean that the buffer sizes have
        // changed! You might need to update internal delay data.
        ret = 1L;
        break;
    case kAsioEngineVersion:
        // return the supported ASIO version of the host application
        // If a host applications does not implement this selector, ASIO 1.0 is
        // assumed by the driver
        ret = 2L;
        break;
    case kAsioSupportsTimeInfo:
        // informs the driver wether the asioCallbacks.bufferSwitchTimeInfo()
        // callback is supported. For compatibility with ASIO 1.0 drivers the
        // host application should always support the "old" bufferSwitch method,
        // too.
        ret = 1;
        break;
    case kAsioSupportsTimeCode:
        // informs the driver wether application is interested in time code
        // info. If an application does not need to know about time code, the
        // driver has less work to do.
        ret = 0;
        break;
    }
    return ret;
