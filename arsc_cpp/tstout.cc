// tstout.cc - test soundcard output 
...
static void
compute_tone(double f, double a, long *b, int n)
{
    ...
    p = 2 * pi * c / n;
    for (i = 0; i < n; i++)
	b[i] = (long) (a * sin(p * i) + 0.5);
}

static void
ramp_ends(long *b, int n, double nr)
{
    double  s, a, p;
    int     i, j, m;
    static double pi = 3.1415927;
...
test_tone(double f, double t)	// tone frequency and duration
{
    ...
    noff = (nsmp * 3) / 2;
    sl = (long *) calloc(nsmp * 2, sizeof(long));
    a = pow(2, nbps * 8 - 1) - 1;
