/* tst3ch.c - test soundcard streaming output on 3 channels */
...
tone_seg(LONG s)
{
	...
	// fill output buffer
	stm[b][i] = (LONG) floor(a * sin(ph) + 0.5);
	// increment time-step
	t += d;
    }
    ph = fmod(ph, tpi);
...
test_tone(double f, double t)	// tone frequency and duration
{
    ...
    LONG siz[2], fmt[2];
    void *out[8];
    static int nchn = 4;        // even number of channels
    ...
    stm[0] = (LONG *) calloc(nsmp, sizeof(LONG));
    stm[1] = (LONG *) calloc(nsmp, sizeof(LONG));
    for (i = 0; i < 3; i++) {
        j = i + nchn;
	out[i] = stm[0];        // stm0 on channels 1,2,3
	out[j] = stm[1];        // stm1 on channels 1,2,3
    }
    out[3] = out[7] = NULL;     // channel 4 not used
    siz[0] = siz[1] = nsmp;
 ...
 */
LRESULT CALLBACK
wind_proc(HWND hWindow, UINT message, WPARAM wParam, LONG lParam)
{
    int seg;
    PAINTSTRUCT   ps;
